...
import { IForm } from "interfaces/Schoolarship";
...
import { useSchoolarshipForm } from "hooks/swr/schoolarship";
...
const Form = ({ id }: { id?: number }) => {
  ...
  const { data, isError, isLoading, mutate } = useSchoolarshipForm(
    reqConfig,
    id
  );
  ...
  const {
    ...
    resolver: yupResolver(
      yup.object({
        title: yup.string().required().label("Judul Beasiswa"),
        ...
        registration_link: yup.string().required().label("Link Registrasi"),
      })
    ),
  });
  ...
  const submit: SubmitHandler<IForm> = async ({
    ...
    let endPoint = "/panel/schoolarships/store";

    if (id) {
      endPoint = `/panel/schoolarships/update/${id}`;
    } else {
      if (!covers.length) {
    ...
    backendApi
      ...
      .then((res) => {
        ...
        setTimeout(() => {
          router.push("/informations/schoolarship");
        }, 2000);
      })
